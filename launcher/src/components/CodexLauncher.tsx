...
export function CodexLauncher({ onOpen, onReady, onRest, inputEnabled }: { onOpen: (panel: Panel) => void; onReady: () => void; onRest: () => void; inputEnabled: boolean }) {
  ...
  return <main className={`codex-launcher ${closing ? "app-exit" : ""}`} style={{ "--focus-bloom": accent, "--px": parallax, "--dir": switchDir } as CSSProperties}>
    ...
    <header className="codex-topbar"><div className="codex-utility"><div className="codex-glass codex-pill-group"><button className={`codex-utility-button codex-wifi-button ${topFocus === "wifi" ? "is-focused" : ""}`} onClick={() => { setTopFocus("wifi"); onOpen("settings"); }} aria-label="Wi-Fi and network"><WifiIcon /><span>Wi-Fi</span></button><button className={`codex-utility-button ${topFocus === "search" ? "is-focused" : ""}`} onClick={() => { setTopFocus("search"); onOpen("search"); }} aria-label="Search"><span className="codex-search-glyph" /><span>Search</span></button><button className={`codex-utility-button ${topFocus === "settings" ? "is-focused" : ""}`} onClick={() => { setTopFocus("settings"); onOpen("settings"); }} aria-label="Settings"><GearIcon /><span>Settings</span></button><button className={`codex-utility-button ${topFocus === "power" ? "is-focused" : ""}`} onClick={() => setPowerOpen(true)} aria-label="Power"><PowerIcon /><span>Power</span></button></div><div className="codex-glass codex-pill-group codex-status-group">{battery !== null && <div className="codex-status" title={`DualSense battery: about ${battery}%`}><BatteryIcon level={battery} charging={charging} /><span>~{battery}%</span></div>}<div className="codex-clock"><Clock /></div></div></div></header>
