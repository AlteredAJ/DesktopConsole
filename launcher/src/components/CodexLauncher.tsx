...
export function CodexLauncher({ onOpen, onReady, onRest, inputEnabled }: { onOpen: (panel: Panel) => void; onReady: () => void; onRest: () => void; inputEnabled: boolean }) {
  ...
  // Tell the HID loop to suppress exit combos while the keyboard overlay is visible.
  useEffect(() => { if (keyboardOpen) void invoke("notify_keyboard_open"); else void invoke("notify_keyboard_closed"); }, [keyboardOpen]);
  ...
  const mountGuardUntil = useRef(Date.now() + 500); // block Cross-launch for 500ms after mount (restore leak guard)
  useEffect(() => {
    let cancelled = false;
  ...
  useEffect(() => { if (!tileIds.length) return; let stopped = false; const poll = () => void invoke<string[]>("running_tile_ids", { tileIds }).then((ids) => { if (!stopped) setRunningIds(new Set(ids)); }); let interval: number | null = null; const startPolling = () => { if (interval !== null) return; poll(); interval = window.setInterval(poll, 4000); }; const stopPolling = () => { if (interval !== null) { window.clearInterval(interval); interval = null; } }; const onVis = () => { if (document.visibilityState === "hidden") stopPolling(); else startPolling(); }; startPolling(); document.addEventListener("visibilitychange", onVis); return () => { stopped = true; stopPolling(); document.removeEventListener("visibilitychange", onVis); }; }, [tileIds]);
  useEffect(() => {
  ...
  useController((pad) => {
    ...
    if (edge.rising("cross")) { if (topFocus === "wifi") { selectFeedback(); onOpen("settings"); } else if (topFocus === "search") { selectFeedback(); onOpen("search"); } else if (topFocus === "settings") { selectFeedback(); onOpen("settings"); } else if (topFocus === "power") { selectFeedback(); setPowerOpen(true); } else if (activeTile) { if (Date.now() < mountGuardUntil.current) return; if (activeTile.category === "games") { selectFeedback(); setActionIndex(0); setExpandedTile(activeTile); } else launch(activeTile); } }
  ...
  return <main className={`codex-launcher ${closing ? "app-exit" : ""}`} style={{ "--focus-bloom": accent, "--px": parallax, "--dir": switchDir } as CSSProperties}>
    <style>{CSS}</style><Atmosphere variant="home" /><FluidAtmosphere preset="midnight" /><div className="codex-backdrop" aria-hidden="true"><div className={`codex-live-backdrop ${showLiveBackdrop ? "is-ready" : ""}`} style={{ backgroundImage: liveBackground }} />{HeroArt && <div className={`codex-hero-art ${showLiveBackdrop ? "is-covered" : ""}`}><HeroArt /></div>}</div>
