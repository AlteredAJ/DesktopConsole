...
export function CodexLauncher({ onOpen, onReady, onRest, inputEnabled }: { onOpen: (panel: Panel) => void; onReady: () => void; onRest: () => void; inputEnabled: boolean }) {
  ...
  const prevHat = useRef(8); const prevStick = useRef<Direction | null>(null); const prevCross = useRef(false); const prevCircle = useRef(false); const prevSquare = useRef(false); const prevShoulders = useRef(0); const dragStartFocus = useRef(0); const dragging = useRef(false); const dragLastDx = useRef(0); const dragLastT = useRef(0); const dragVel = useRef(0);
  useEffect(() => {
    let cancelled = false;
  ...
  useEffect(() => () => { if (outgoingTimer.current) window.clearTimeout(outgoingTimer.current); }, []);
  ...
  function setTab(next: number) {
    ...
    if (!reduceMotion.current && tiles.length) {
      ...
      outgoingTimer.current = window.setTimeout(() => setOutgoing(null), 240);
    }
    setTabIndex(clamped);
    navFeedback();
  }
