...
export function CodexLauncher({ onOpen, onReady, onRest, inputEnabled }: { onOpen: (panel: Panel) => void; onReady: () => void; onRest: () => void; inputEnabled: boolean }) {
  ...
  function launch(tile?: Tile) { if (!tile) return; recordLaunch(tile.id); setRecents(getRecents()); launchFeedback();
    ...
    duckAmbient(true); setClosing(true); window.setTimeout(() => void invoke("launch_app", { target: tile.id, needsCursor: !!tile.needsCursor }), 160); }
  ...
  return <main className={`codex-launcher ${closing ? "app-exit" : ""}`} style={{ "--focus-bloom": accent, "--px": parallax, "--dir": switchDir } as CSSProperties}>
    ...
    <header className="codex-nav">
      <div className="codex-nav-left">
        <div className="codex-nav-icon" aria-label="Controller">
          <svg viewBox="0 0 64 54" fill="none" stroke="#e4ecff" strokeWidth="2.6" strokeLinecap="round" strokeLinejoin="round"><path d="M8 12h48c4 0 4 4 4 4v8c0 16-4 22-14 22-6 0-12-4-14-12-2 8-8 12-14 12C8 46 4 40 4 24v-8c0 0 0-4 4-4Z"/><rect x="22" y="14" width="20" height="12" rx="4"/><line x1="24" y1="6" x2="40" y2="6" stroke="#5b9cf5" strokeWidth="2.5"/><circle cx="29" cy="32" r="4"/><circle cx="35" cy="32" r="4"/></svg>
        </div>
        <div className="codex-nav-tabs">{tabGlow && <span className={`codex-tab-glow ${tabGlowMoving ? "is-moving" : ""}`} aria-hidden="true" style={{ left: tabGlow.left, width: tabGlow.width }} />}{TABS.map((tab, index) => <button ref={(node) => { tabRefs.current[index] = node; }} key={tab.id} className={`codex-tab ${tabIndex === index ? "active" : ""}`} onClick={() => setTab(index)}>{tab.label}</button>)}</div>
      </div>
      <div className="codex-nav-right">
        <div className="codex-nav-utils">{utilGlow && <span className={`codex-util-glow ${utilGlowMoving ? "is-moving" : ""}`} aria-hidden="true" style={{ left: utilGlow.left, width: utilGlow.width }} />}<button ref={(node) => { utilRefs.current[0] = node; }} className="codex-nav-btn" onClick={() => { setTopFocus("wifi"); onOpen("settings"); }} aria-label="Wi-Fi"><WifiIcon /></button><button ref={(node) => { utilRefs.current[1] = node; }} className="codex-nav-btn" onClick={() => { setTopFocus("search"); onOpen("search"); }} aria-label="Search"><span className="codex-search-glyph" /></button><button ref={(node) => { utilRefs.current[2] = node; }} className="codex-nav-btn" onClick={() => { setTopFocus("settings"); onOpen("settings"); }} aria-label="Settings"><GearIcon /></button><button ref={(node) => { utilRefs.current[3] = node; }} className="codex-nav-btn" onClick={() => setPowerOpen(true)} aria-label="Power"><PowerIcon /></button></div>
        <div className="codex-nav-status">{battery !== null && <span title={`DualSense battery: about ${battery}%`}><BatteryIcon level={battery} charging={charging} /><span>~{battery}%</span></span>}<div className="codex-clock"><Clock /></div></div>
      </div>
    </header>
    {/* A1: top-aligned tile dock */}
    <section className="codex-dock-wrap" aria-label={`${TABS[tabIndex].label} dock`}><div ref={dockRef} className="codex-dock"><div className="codex-dock-light" aria-hidden="true" /><div className="codex-shelf codex-shelf-in" key={tabId}>{tabId === "continue" && tiles.length === 0 ? <div className="codex-empty">Nothing recent yet � launch an app or game and it'll show up here.</div> : tiles.map((tile, index) => <button ref={(node) => { tileRefs.current[index] = node; }} key={`${tile.category}-${index}-${tile.id}`} className={`codex-tile ${focus === index ? "focused" : ""}`} onClick={() => { setFocus(index); if (tile.category === "games") { setActionIndex(0); setExpandedTile(tile); } else launch(tile); }}><span className="codex-tile-icon"><ServiceIcon id={tile.id} /></span><span className="codex-tile-label">{tile.label}</span>{runningIds.has(tile.id) && <i className="codex-running" aria-label="Running" />}</button>)}</div>{outgoing && <div className="codex-shelf codex-shelf-ghost" style={{ "--dir": outgoing.dir } as CSSProperties} aria-hidden="true">{outgoing.tiles.map((tile, index) => <span key={`${tile.category}-${index}-${tile.id}`} className="codex-tile"><span className="codex-tile-icon"><ServiceIcon id={tile.id} /></span><span className="codex-tile-label">{tile.label}</span></span>)}</div>}</div></section>
    {/* A1: game detail area � slides up below dock when a tile is selected */}
    {activeTile && <section className="codex-detail"><div className="codex-detail-left"><div className="codex-copy" key={`${activeTile.category}-${activeTile.id}-${activeTile.label}`} style={{ "--dir": copyDir, "--copy-blur": copyDir ? "9px" : "0px" } as CSSProperties}><div className="codex-eyebrow">{runningIds.has(activeTile.id) ? <><i className="codex-live-dot" style={{ background: "#1AFF84", boxShadow: "0 0 1.2cqh #1AFF84" }} />NOW PLAYING</> : <><i className="codex-live-dot" />{activeTile.category.toUpperCase()}</>}</div><h1>{activeTile.label}</h1><p>{taglineFor(activeTile.id) || (activeTile.category === "games" ? "Ready when you are" : "Open and continue")}</p><div className="codex-detail-actions">{activeTile.category === "games" ? <><button className="codex-cta-play" onClick={() => { setActionIndex(0); setExpandedTile(activeTile); }}>Play Game</button><button className="codex-cta-more" onClick={() => { setActionIndex(0); setExpandedTile(activeTile); }}><span /><span /><span /></button></> : <button className="codex-cta-play" onClick={() => launch(activeTile)}>Launch</button>}{runningIds.has(activeTile.id) && <button className="codex-cta-close" onClick={() => setConfirmClose(activeTile)}><GlyphSquare /> Close</button>}</div></div></div><div className="codex-detail-right">{activeTile.category === "games" ? <div className="codex-detail-card"><div className="codex-detail-card-inner"><ServiceIcon id={activeTile.id} /></div></div> : null}</div></section>}
    {/* Hints � bottom-center */}
    <footer className="codex-hints" aria-label="Controller hints">
      <span className="codex-hint"><b>D-pad / Stick</b> Navigate</span>
      <span className="codex-hint"><i className="codex-hint-icon"><GlyphSwipe /></i> Browse</span>
      <span className="codex-hint"><b>L1 / R1</b> Switch tab</span>
      {activeTile && <span className="codex-hint"><i className="codex-hint-icon"><GlyphCross /></i>Select</span>}
      {activeTile && runningIds.has(activeTile.id) && <span className="codex-hint"><i className="codex-hint-icon"><GlyphSquare /></i>Close</span>}
      <span className="codex-hint"><i className="codex-hint-icon"><GlyphOptions /></i>Menu</span>
    </footer>
    {powerOpen && <div className="codex-power-backdrop" onClick={() => setPowerOpen(false)}><section className="codex-power-panel" style={{ borderColor: "rgba(224,242,255,.22)", boxShadow: "-24px 0 80px rgba(0,0,0,.65), inset 1px 0 rgba(255,255,255,.18)", backdropFilter: "blur(38px) saturate(160%)", WebkitBackdropFilter: "blur(38px) saturate(160%)" }} onClick={(e) => e.stopPropagation()}>{powerConfirm ? <><div className="codex-eyebrow">CONFIRM</div><h2>{POWER_ACTIONS[powerIndex].label}?</h2><p>{POWER_ACTIONS[powerIndex].description}</p><div className="codex-power-confirm"><button onClick={commitPower}><i className="codex-dualsense-inline"><GlyphCross /></i>Confirm</button><button onClick={() => setPowerConfirm(false)}><i className="codex-dualsense-inline"><GlyphCircle /></i>Cancel</button></div></> : <><style>{POWER_CSS}</style><div className="codex-eyebrow"><i className="codex-live-dot" />POWER</div><h2>End your session</h2><p className="codex-power-sub">Choose an action � destructive ones require confirmation.</p><div className="codex-power-actions">{POWER_ACTIONS.map((action, index) => <button key={action.id} className={`codex-power-action ${index === powerIndex ? "selected" : ""}`} onClick={() => { setPowerIndex(index); commitPower(); }}>{action.id === "minimize" ? <><span className="codex-power-action-icon">&#9646;</span><span className="codex-power-action-info"><b>Minimize console</b><s>Hide and return to Windows</s></span></> : action.id === "close" ? <><span className="codex-power-action-icon">&#10005;</span><span className="codex-power-action-info"><b>Close console</b><s>Exit console mode</s></span></> : action.id === "lock" ? <><span className="codex-power-action-icon">&#128274;</span><span className="codex-power-action-info"><b>Lock console</b><s>Require Windows sign-in</s></span></> : action.id === "sleep" ? <><span className="codex-power-action-icon">&#9790;</span><span className="codex-power-action-info"><b>Rest mode</b><s>Idle screen, lower power use</s></span></> : <><span className="codex-power-action-icon">&#9211;</span><span className="codex-power-action-info"><b>Turn off</b><s>Power down this PC</s></span></>}<span className="codex-power-action-glyph"><GlyphCross /></span></button>)}</div><div className="codex-power-footer">Up / Down to choose <b><GlyphCross /> Select</b> <span><GlyphCircle /> Cancel</span></div></>}</section></div>}
    ...
    {expandedTile && <GameDetailPanel tile={expandedTile} actions={expandedActions} actionIndex={actionIndex} isRunning={runningIds.has(expandedTile.id)} onAction={(action) => runGameAction(action)} onClose={() => setExpandedTile(null)} />}
...
const CSS = `
.codex-launcher{position:fixed;inset:0;overscroll-behavior:none;isolation:isolate;overflow:hidden;width:100%;height:100%;color:#f4f6fb;background:#090b10;font-family:"Manrope",system-ui,sans-serif;container-type:size}
.codex-backdrop{position:absolute;inset:-5cqw;z-index:-1;overflow:hidden;pointer-events:none;transform:translate3d(calc(var(--px,0)*2.4cqw),0,0);transition:transform .55s cubic-bezier(.22,1,.36,1)}
.codex-launcher .atmos{transform:translate3d(calc(var(--px,0)*1.1cqw),0,0);transition:transform .6s cubic-bezier(.22,1,.36,1)}
.codex-live-backdrop{position:absolute;inset:0;opacity:0;transform:scale(1.035);background-position:center;background-size:cover;filter:saturate(.78) contrast(1.05);transition:opacity .32s cubic-bezier(.22,1,.36,1),transform .7s cubic-bezier(.22,1,.36,1);will-change:opacity,transform}
.codex-live-backdrop.is-ready{opacity:1;transform:scale(1)}
.codex-hero-art{position:absolute;inset:-10%;opacity:1;animation:codex-fade .5s ease both;transition:opacity .27s ease}
.codex-hero-art.is-covered{opacity:0}
.codex-nav{position:absolute;top:0;left:0;right:0;height:108px;display:flex;align-items:center;justify-content:space-between;padding:0 4.2cqw;z-index:20}
.codex-nav-left{display:flex;align-items:center;gap:2.2cqw}
.codex-nav-icon{width:6.4cqh;height:6.4cqh;border-radius:1.2cqh;background:radial-gradient(circle at 42% 38%,#2a3d6e,#0f1830);display:flex;align-items:center;justify-content:center;border:1px solid rgba(255,255,255,.1);flex-shrink:0}
.codex-nav-icon svg{width:4.2cqh;height:3.6cqh}
...
.codex-nav-utils{position:relative;display:flex;align-items:center;gap:.45cqw}
.codex-util-glow{position:absolute;top:.32cqh;height:3.86cqh;border-radius:1.1cqh;pointer-events:none;z-index:0;background:linear-gradient(180deg,color-mix(in srgb,var(--focus-bloom) 30%,rgba(255,255,255,.16)),color-mix(in srgb,var(--focus-bloom) 10%,rgba(255,255,255,.03)));box-shadow:0 0 1.8cqh color-mix(in srgb,var(--focus-bloom) 50%,transparent),inset 0 1px rgba(255,255,255,.4);transition:left .3s cubic-bezier(.22,1,.36,1),width .3s cubic-bezier(.22,1,.36,1)}
.codex-util-glow.is-moving{filter:blur(6px) saturate(150%);transform:scaleX(1.12)}
.codex-nav-btn{position:relative;z-index:1;height:3.86cqh;display:flex;align-items:center;justify-content:center;background:none;border:0;color:inherit;font:inherit;padding:0 .9cqw;cursor:pointer}
.codex-nav-btn svg{width:1.8cqh;height:1.8cqh;fill:none;stroke:#e4ecf8;stroke-width:2;stroke-linecap:round;stroke-linejoin:round}
.codex-search-glyph{width:1.6cqh;height:1.6cqh;border:2px solid #e4ecf8;border-radius:50%;position:relative;display:block}
.codex-search-glyph:after{content:"";position:absolute;width:.75cqh;height:2px;background:#e4ecf8;right:-.52cqh;bottom:-.26cqh;transform:rotate(45deg);border-radius:1px}
.codex-nav-status{display:flex;align-items:center;gap:.8cqw;font-size:1.05cqh;font-weight:600;color:rgba(255,255,255,.7);margin-left:.5cqw}
.codex-nav-status .codex-clock{font-size:1.05cqh;font-weight:300;color:#bcc8de}
.codex-dock-wrap{position:absolute;top:13cqh;left:4.2cqw;right:4.2cqw;z-index:15}
.codex-dock{overscroll-behavior-x:contain;overflow-x:auto;overflow-y:hidden;scrollbar-width:none;padding:1.2cqh 0}
.codex-dock::-webkit-scrollbar{display:none}
.codex-dock-light{position:absolute;inset:0;pointer-events:none;background:linear-gradient(100deg,transparent 20%,color-mix(in srgb,var(--focus-bloom) 16%,transparent) 50%,transparent 78%);filter:blur(1.2cqh)}
.codex-shelf{position:relative;display:flex;align-items:flex-start;gap:.7cqw;width:max-content;padding:.4cqh .6cqw}
.codex-shelf-in{animation:codex-shelf-in .32s cubic-bezier(.22,1,.36,1) both}
...
.codex-tile.focused .codex-tile-icon{font-size:4cqh;padding-top:1.2cqh}
.codex-tile-label{font-size:1.1cqh;font-weight:600;color:rgba(255,255,255,.55);text-align:center;max-width:90%;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;padding-bottom:.8cqh}
.codex-tile.focused .codex-tile-label{color:rgba(255,255,255,.85)}
.codex-running{position:absolute;top:.8cqh;right:.8cqh;width:.7cqh;height:.7cqh;border-radius:50%;background:#1AFF84;box-shadow:0 0 5px rgba(26,255,132,.5)}
.codex-detail{position:absolute;top:30cqh;left:4.2cqw;right:4.2cqw;bottom:10cqh;display:flex;gap:3.2cqw;z-index:10;animation:codex-detail-up .28s cubic-bezier(.22,1,.36,1) both}
@keyframes codex-detail-up{from{opacity:0;transform:translateY(1.6cqh)}to{opacity:1;transform:none}}
.codex-detail-left{flex:1;display:flex;flex-direction:column;justify-content:flex-end;min-width:0}
.codex-detail-right{width:26cqw;flex-shrink:0;display:flex;flex-direction:column;justify-content:flex-end}
.codex-copy{text-shadow:0 3px 24px rgba(0,0,0,.65);animation:codex-copy-in .26s cubic-bezier(.22,1,.36,1) both}
.codex-eyebrow{display:flex;align-items:center;gap:.7cqw;font-size:1.02cqh;font-weight:800;letter-spacing:.18em;color:rgba(255,255,255,.6)}
.codex-live-dot{width:.66cqh;height:.66cqh;border-radius:50%;background:var(--focus-bloom);box-shadow:0 0 1.2cqh var(--focus-bloom)}
.codex-copy h1{margin:1.6cqh 0 1cqh;font-size:7.2cqh;line-height:.92;font-weight:900;letter-spacing:-.05em;max-width:54cqw}
.codex-copy p{margin:0;font-size:1.9cqh;font-weight:560;letter-spacing:-.01em;color:rgba(255,255,255,.7);max-width:48cqw;line-height:1.4}
.codex-detail-actions{display:flex;align-items:center;gap:1.2cqw;margin-top:2.4cqh}
.codex-cta-play{width:28cqw;height:6.2cqh;border-radius:3.1cqh;border:1px solid rgba(255,255,255,.18);background:rgba(255,255,255,.11);backdrop-filter:blur(16px) saturate(150%);-webkit-backdrop-filter:blur(16px) saturate(150%);color:#fff;font:700 1.6cqh "Manrope",system-ui,sans-serif;cursor:pointer;box-shadow:0 .6cqh 2.2cqh rgba(0,0,0,.4),inset 0 1px rgba(255,255,255,.3);transition:background .15s;letter-spacing:-.01em}
.codex-cta-play:hover{background:rgba(255,255,255,.18)}
.codex-cta-more{width:6.2cqh;height:6.2cqh;border-radius:50%;border:1px solid rgba(255,255,255,.18);background:rgba(255,255,255,.11);backdrop-filter:blur(16px) saturate(150%);-webkit-backdrop-filter:blur(16px) saturate(150%);display:flex;align-items:center;justify-content:center;gap:5px;cursor:pointer;box-shadow:0 .6cqh 2.2cqh rgba(0,0,0,.4),inset 0 1px rgba(255,255,255,.3)}
.codex-cta-more span{width:.5cqh;height:.5cqh;border-radius:50%;background:#fff}
.codex-cta-close{height:6.2cqh;padding:0 1.6cqw;border-radius:1.4cqh;border:1px solid rgba(255,255,255,.1);background:rgba(255,255,255,.05);color:rgba(255,255,255,.6);font:600 1.3cqh "Manrope",sans-serif;cursor:pointer;display:flex;align-items:center;gap:.6cqw;transition:background .15s}
.codex-cta-close:hover{background:rgba(220,60,60,.1);border-color:rgba(220,60,60,.15);color:#f08080}
.codex-detail-card{width:100%;aspect-ratio:1;border-radius:1.6cqh;overflow:hidden;border:1px solid rgba(255,255,255,.1);background:linear-gradient(145deg,rgba(255,255,255,.05),rgba(0,0,0,.3));display:flex;align-items:center;justify-content:center;box-shadow:0 .8cqh 2.6cqh rgba(0,0,0,.4)}
.codex-detail-card-inner{font-size:5cqh;color:rgba(255,255,255,.06)}
.codex-hints{position:absolute;bottom:1.6cqh;left:50%;transform:translateX(-50%);display:flex;gap:1.2cqw;z-index:20}
.codex-hint{padding:.7cqh 1.4cqw;border-radius:1.2cqh;background:rgba(255,255,255,.06);backdrop-filter:blur(12px);-webkit-backdrop-filter:blur(12px);border:1px solid rgba(255,255,255,.06);font-size:1.1cqh;font-weight:600;color:rgba(255,255,255,.45);display:flex;align-items:center;gap:.7cqw}
.codex-hint b{color:rgba(255,255,255,.75);font-weight:700;font-size:1.3cqh;margin-right:.2cqw}
.codex-hint-icon{display:flex;align-items:center;font-style:normal;font-size:1.3cqh;opacity:.6}
.codex-hint-icon svg{width:1.4cqh;height:1.4cqh}
.codex-empty{color:rgba(255,255,255,.25);font-size:1.2cqh;font-weight:600;padding:2cqh 0;text-align:center;width:100%}
.codex-confirm-backdrop{position:fixed;inset:0;z-index:40;background:rgba(2,4,8,.6);backdrop-filter:blur(8px);-webkit-backdrop-filter:blur(8px);display:grid;place-items:center}
.codex-confirm{width:min(38cqw,30rem);padding:3cqh 2.8cqw;border-radius:2cqh}
.codex-confirm h2{font-size:2.2cqh;margin:.8cqh 0}
.codex-confirm p{color:rgba(255,255,255,.55);margin-bottom:1.6cqh;font-size:1.2cqh}
.codex-confirm div{display:flex;gap:1cqw}
@keyframes codex-copy-in{from{opacity:0;transform:translateY(1.2cqh)}to{opacity:1;transform:none}}
@keyframes codex-fade{from{opacity:0}to{opacity:1}}
@media(prefers-reduced-motion:reduce){.codex-detail,.codex-power-panel,.codex-copy,.codex-shelf-in{animation:none!important}.codex-tile,.codex-tab-glow,.codex-util-glow{transition:none!important}}
html.reduce-motion .codex-detail,html.reduce-motion .codex-power-panel,html.reduce-motion .codex-copy{animation:none!important}html.reduce-motion .codex-tile,html.reduce-motion .codex-tab-glow,html.reduce-motion .codex-util-glow{transition:none!important}
`;


const POWER_CSS = `
.codex-power-backdrop{position:fixed;inset:0;z-index:50;background:linear-gradient(90deg,transparent 60%,rgba(2,4,9,.32) 85%,rgba(2,4,9,.55))}
.codex-power-panel{position:absolute;right:0;top:0;bottom:0;width:min(44cqw,560px);z-index:51;display:flex;flex-direction:column;padding:42px 44px;
  background:linear-gradient(110deg,rgba(18,24,42,.92),rgba(10,14,28,.94) 40%,rgba(8,11,22,.96));
  border-left:1px solid rgba(255,255,255,.14);overflow:hidden;
  animation:codex-power-slide .3s cubic-bezier(.22,1,.36,1) both}
.codex-power-panel::before{content:"";position:absolute;z-index:-1;inset:-20% -10% auto -10%;height:50%;
  background:radial-gradient(ellipse at 15% 8%,rgba(255,255,255,.14),transparent 55%),radial-gradient(ellipse at 85% 5%,rgba(100,160,240,.06),transparent 50%)}
.codex-power-panel::after{content:"";position:absolute;z-index:-1;right:0;bottom:0;width:60%;height:40%;
  background:radial-gradient(ellipse at 100% 100%,color-mix(in srgb,var(--focus-bloom) 10%,transparent),transparent 70%)}
.codex-power-panel .codex-eyebrow{margin-bottom:10px}.codex-power-panel h2{font-size:3.2cqh;margin-bottom:4px}
.codex-power-sub{font-size:1.4cqh;color:rgba(255,255,255,.38);margin-bottom:28px}
.codex-power-actions{display:flex;flex-direction:column;gap:5px;flex:1;overflow-y:auto}
.codex-power-action{display:flex;align-items:center;padding:12px 16px;border-radius:14px;cursor:pointer;border:1px solid transparent;background:none;color:inherit;font:inherit;text-align:left;transition:background .15s}
.codex-power-action:hover,.codex-power-action.selected{background:linear-gradient(105deg,color-mix(in srgb,var(--focus-bloom) 30%,rgba(255,255,255,.12)),rgba(255,255,255,.08));border-color:color-mix(in srgb,var(--focus-bloom) 18%,transparent);box-shadow:0 0 18px color-mix(in srgb,var(--focus-bloom) 18%,transparent),inset 0 1px rgba(255,255,255,.25)}
.codex-power-action-icon{width:38px;height:38px;border-radius:10px;display:flex;align-items:center;justify-content:center;font-size:1.8cqh;font-style:normal;flex-shrink:0;margin-right:14px;background:rgba(255,255,255,.06);border:1px solid rgba(255,255,255,.08)}
.codex-power-action-info{flex:1;min-width:0}
.codex-power-action-info b{display:block;font-size:1.5cqh;font-weight:700;letter-spacing:-.01em}
.codex-power-action-info s{font-size:1.1cqh;color:rgba(255,255,255,.35);text-decoration:none;display:block;margin-top:1px;line-height:1.3;font-weight:400}
.codex-power-action-glyph{font-size:1.4cqh;opacity:.35;font-weight:600}
.codex-power-confirm{display:flex;flex-direction:column;align-items:center;gap:16px;flex:1;justify-content:center}
.codex-power-confirm p{font-size:1.3cqh;color:rgba(255,255,255,.5)}
.codex-power-footer{border-top:1px solid rgba(255,255,255,.04);padding-top:16px;display:flex;gap:18px;font-size:1.1cqh;color:rgba(255,255,255,.32)}
.codex-power-footer b{color:rgba(255,255,255,.6)}
@keyframes codex-power-slide{from{opacity:0;transform:translateX(40px)}to{opacity:1;transform:none}}
@media(prefers-reduced-motion:reduce){.codex-power-panel{animation:none}}`;