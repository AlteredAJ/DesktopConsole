...
export function CodexLauncher({ onOpen, onReady, onRest, inputEnabled }: { onOpen: (panel: Panel) => void; onReady: () => void; onRest: () => void; inputEnabled: boolean }) {
  ...
  return <main className={`codex-launcher ${closing ? "app-exit" : ""}`} style={{ "--focus-bloom": accent, "--px": parallax, "--dir": switchDir } as CSSProperties}>
    ...
    {/* Hints � bottom-center */}
    <footer className="codex-hints" aria-label="Controller hints"><span className="codex-hint"><b>D-pad / Stick</b> Navigate</span><span className="codex-hint"><i><GlyphSwipe /></i> Browse</span><span className="codex-hint"><b>L1 / R1</b> Switch tab</span>{activeTile && <span className="codex-hint"><i className="codex-hint-icon"><GlyphCross /></i>Select</span>}{activeTile && runningIds.has(activeTile.id) && <span className="codex-hint"><i className="codex-hint-icon"><GlyphSquare /></i>Close</span>}<span className="codex-hint"><i className="codex-hint-icon"><GlyphOptions /></i>Menu</span></footer>
...
.codex-hints{position:absolute;bottom:1.6cqh;left:50%;transform:translateX(-50%);display:flex;gap:.8cqw;z-index:20}
.codex-hint{padding:.6cqh 1.2cqw;border-radius:1.1cqh;background:rgba(255,255,255,.05);backdrop-filter:blur(10px);-webkit-backdrop-filter:blur(10px);border:1px solid rgba(255,255,255,.06);font-size:1cqh;font-weight:600;color:rgba(255,255,255,.38);display:flex;align-items:center;gap:.6cqw}
.codex-hint b{color:rgba(255,255,255,.65);font-weight:700;margin-right:.2cqw}
.codex-hint-icon{display:flex;align-items:center;font-style:normal}
