...
/**
 * Summonable keyboard overlay for Home — dims the backdrop and floats
 * VirtualKeyboard's `bare` core (swipe-to-select grid + D-pad/stick nav +
 * Cross-commit) in a glass panel instead of its full-screen panel shell.
 ...
 */
export function KeyboardOverlay({ title = "Keyboard", subtitle, placeholder, secret, onDone, onCancel }: KeyboardOverlayProps) {
  return <div className="codex-keyboard-overlay">
    <style>{CSS}</style>
    <section className="codex-keyboard-overlay-panel">
      <header className="codex-keyboard-overlay-header"><div className="codex-eyebrow">TEXT INPUT</div><h2>{title}</h2>{subtitle && <p>{subtitle}</p>}</header>
      <VirtualKeyboard bare placeholder={placeholder} secret={secret} onDone={onDone} onCancel={onCancel} />
    </section>
  </div>;
}
...
@media (prefers-reduced-motion: reduce){.codex-keyboard-overlay{animation:none}}
`;
